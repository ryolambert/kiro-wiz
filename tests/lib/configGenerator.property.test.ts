...
import type { HookConfig, PowerConfig, SkillFrontmatter, SteeringConfig } from '../../lib/types.js';
...
const arbTriggerType: fc.Arbitrary<HookConfig['when']['type']> = fc.constantFrom(
  ...VALID_HOOK_TRIGGERS,
);

const arbActionType: fc.Arbitrary<HookConfig['then']['type']> = fc.constantFrom(
  ...VALID_HOOK_ACTIONS,
);

const arbInclusionMode: fc.Arbitrary<SteeringConfig['inclusion']> = fc.constantFrom(
  ...VALID_INCLUSION_MODES,
);
 ...
 * and optional fields.
 */
const arbSkillFrontmatter: fc.Arbitrary<SkillFrontmatter> = fc.record({
  name: arbSkillName,
  description: arbSkillDescription,
  license: fc.option(arbNonEmptyString, { nil: undefined }),
  compatibility: fc.option(fc.string({ minLength: 1, maxLength: 100 }), { nil: undefined }),
  metadata: fc.option(
    fc.dictionary(
      fc.string({
...
const arbHookOptions = fc
  .tuple(arbNonEmptyString, arbTriggerType, arbActionType, arbNonEmptyString, arbNonEmptyString)
...
describe('Property 22: Generated config validity', () => {
  ...
  it('generateHook() askAgent action always has prompt', () => {
    fc.assert(
      fc.property(arbNonEmptyString, arbTriggerType, arbNonEmptyString, (name, trigger, prompt) => {
  ...
  it('generateSteering() output parses back to valid config', () => {
    fc.assert(
      fc.property(arbSteeringConfig, (steeringConfig) => {
        ...
        expect(markdown).toContain(`inclusion: ${steeringConfig.inclusion}`);
  ...
  it('generateSteering() fileMatch mode includes fileMatchPattern', () => {
    fc.assert(
      fc.property(arbNonEmptyString, arbNonEmptyString, (content, pattern) => {
  ...
  it('generateSkill() output passes validate() for valid frontmatter', () => {
    fc.assert(
      fc.property(arbSkillFrontmatter, arbNonEmptyString, (frontmatter, body) => {
        ...
        expect(result.skillMd).toContain(`name: ${frontmatter.name}`);
  ...
  it('generateSkill() produces directory structure with skill name', () => {
    fc.assert(
      fc.property(arbSkillFrontmatter, arbNonEmptyString, (frontmatter, body) => {
        ...
        expect(result.directories).toContain(frontmatter.name);
      }),
      { numRuns: 100 },
    );
  });
    ...
    fc.assert(
      fc.property(
        ...
        (frontmatter, body, scripts, refs, assets) => {
          ...
          if (scripts) {
            expect(result.directories).toContain(`${frontmatter.name}/scripts`);
          }

          if (refs) {
            expect(result.directories).toContain(`${frontmatter.name}/references`);
          }

          if (assets) {
            expect(result.directories).toContain(`${frontmatter.name}/assets`);
          }
        },
      ),
      { numRuns: 100 },
  ...
  it('generateMcpConfig() produces valid mcp.json for local servers', () => {
    fc.assert(
      fc.property(
        ...
        (name, command, args) => {
          const result = generateMcpConfig([{ name, type: 'local', command, args }]);
  ...
  it('generateMcpConfig() produces valid mcp.json for remote servers', () => {
    fc.assert(
      fc.property(arbNonEmptyString, arbNonEmptyString, (name, url) => {
        const result = generateMcpConfig([{ name, type: 'remote', url }]);
    ...
    fc.assert(
      fc.property(
        ...
        (localName, command, remoteName, url) => {
          ...
          const rName = localName === remoteName ? `${remoteName}-remote` : remoteName;
          ...
          expect('command' in result.mcpServers[localName]).toBe(true);
          expect('url' in result.mcpServers[rName]).toBe(true);
        },
      ),
      { numRuns: 100 },
    );
...
const arbMcpServerEntry = fc.oneof(
  arbLocalMcpServer.map(
    (s) =>
      s as { command: string; args: string[] } | { url: string; headers?: Record<string, string> },
  ),
...
describe('Property 23: Power structure completeness', () => {
  it('generatePower() produces POWER.md with valid frontmatter containing name, displayName, description, keywords', () => {
    fc.assert(
      fc.property(arbPowerOptions, (opts) => {
        ...
        expect(result.powerMd).toContain(`displayName: ${opts.displayName}`);
        expect(result.powerMd).toContain(`description: ${opts.description}`);
        expect(result.powerMd).toContain(`keywords: ${opts.keywords.join(', ')}`);

        // Frontmatter is properly closed
        const fmEnd = result.powerMd.indexOf('---', 4);
  ...
  it('generatePower() includes valid mcp.json when mcpServers are provided', () => {
    fc.assert(
      fc.property(arbPowerOptions, arbMcpServers, (opts, mcpServers) => {
        ...
        for (const [key, server] of Object.entries(result.mcpJson?.mcpServers)) {
  ...
  it('generatePower() produces steering files with valid inclusion mode frontmatter when steeringFiles provided', () => {
    fc.assert(
      fc.property(arbPowerOptions, arbSteeringFiles, (opts, steeringFiles) => {
        const result = generatePower({
          ...opts,
          steeringFiles,
        });
        ...
        expect(result.steeringFiles).toHaveLength(steeringFiles.length);
        ...
        for (let i = 0; i < result.steeringFiles.length; i++) {
          ...
          expect(sf.content).toContain(`inclusion: ${input.inclusion}`);
          expect(validModes.includes(input.inclusion)).toBe(true);

          // fileMatch mode includes pattern
          if (input.inclusion === 'fileMatch') {
            expect(sf.content).toContain('fileMatchPattern:');
          }
