...
export function QueryScreen({ onBack: _onBack }: Props) {
  ...
  useEffect(() => {
    if (!query.trim()) {
      setResults(categories.flatMap(c => c.files.map(f => ({ category: c.category, file: f }))));
    } else {
  ...
  if (selected) {
    return (
      <box style={{ flexDirection: 'column', padding: 1 }}>
        <text fg="#00FFAA"><strong>{selected.title}</strong></text>
        <text fg="#666666">Source: {selected.sourceUrl} | Updated: {selected.lastUpdated}</text>
  ...
  return (
    <box style={{ flexDirection: 'column', padding: 1 }}>
      <text fg="#00FFAA"><strong>Query KB</strong> — Search knowledge base ({results.length} entries)</text>
      <text fg="#666666">ESC to go back</text>
      <box title="Search" style={{ border: true, height: 3, width: 50, marginTop: 1 }}>
        <input placeholder="Filter..." focused={true} onInput={setQuery} onSubmit={async () => {
          if (results.length > 0) {
            const r = results[0];
      ...
      <scrollbox style={{ rootOptions: { backgroundColor: '#1a1a26' }, marginTop: 1 }}>
        {results.map((r, i) => (
          <text key={i} fg="#AAAAAA">{r.category}/{r.file}</text>
        ))}
        {results.length === 0 && <text fg="#888888">No entries found. Run "kiro-wiz sync --all" to populate.</text>}
      </scrollbox>
    </box>
  );
}
