...
function masterRefLink(t: KiroToolType): string {
  return (
    `[${displayName(t)} in Master Reference]` +
    `(../master-reference.md#${toolAnchor(t)})`
  );
}
...
export function generateBestPracticesContent(
...
export function generateExamplesContent(
  toolType: KiroToolType
): DocContent {
  return {
...
export function generateTemplatesContent(
  toolType: KiroToolType
): DocContent {
  return {
...
export function buildCrossRefSection(
  toolType: KiroToolType,
  docType: string
): string {
