...
export function gatherRequirements(input: {
  ...
  const automationGoals = input.automationGoals
    .map((g) => g.trim())
    .filter((g) => g.length > 0);

  const workflowGoals = input.workflowGoals
    .map((g) => g.trim())
    .filter((g) => g.length > 0);

  const preferredPlatform = VALID_PLATFORMS.includes(
...
export function buildIntegrationPlan(
  ...
  const compositePackage =
    requirements.preferredPlatform === 'both';

  return { recommendations, files, compositePackage };
}

function buildScenarioString(
  requirements: IntegrationRequirements
): string {
...
export function buildCompositePackage(
  requirements: IntegrationRequirements
): ScaffoldResult {
...
export function generateBuildSummary(
  ...
  const testingInstructions = buildTestingInstructions(
    plan,
    scaffoldResult
  );
...
function buildTestingInstructions(
  plan: IntegrationPlan,
  scaffoldResult: ScaffoldResult
): string {
  const lines: string[] = ['## Testing Instructions', ''];

  const hasPower = scaffoldResult.files.some((f) =>
    f.path.endsWith('POWER.md')
  );
  const hasAgent = scaffoldResult.files.some(
    (f) =>
      f.path.endsWith('.json') && f.path.includes('agents/')
  );
  const hasMcpJson = scaffoldResult.files.some((f) =>
  ...
  lines.push(
    '### Generated Files',
    ...scaffoldResult.files.map((f) => `- \`${f.path}\``),
    ''
  );
