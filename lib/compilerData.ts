...
const CATEGORY_TO_TOOL_TYPE: Partial<
  Record<UrlCategory, KiroToolType>
> = {
  specs: 'spec',
  hooks: 'hook',
...
export function categoryToToolType(
...
> = {
  spec: {
    ...
    whenToUse:
      'Complex features needing formal planning and tracking',
    whenNotToUse: 'Quick fixes or simple changes',
    alternatives: ['steering-doc', 'skill'],
  },
  hook: {
    whatItIs:
      'Automated actions triggered by IDE/CLI events',
    whenToUse:
      'Automating repetitive tasks on file save, create, or delete',
    whenNotToUse:
      'One-time operations or complex multi-step workflows',
    alternatives: ['steering-doc', 'custom-agent'],
  },
  'steering-doc': {
    whatItIs:
      'Markdown instructions loaded into agent context',
    whenToUse:
      'Providing persistent coding guidelines or project rules',
    whenNotToUse:
      'Dynamic behavior that needs runtime logic',
    alternatives: ['skill', 'hook'],
  },
  skill: {
    whatItIs:
      'Portable agent skills following the Agent Skills spec',
    whenToUse:
      'Reusable domain knowledge shareable across agents',
    whenNotToUse:
      'IDE-specific UI integrations or one-off scripts',
    alternatives: ['steering-doc', 'power'],
  },
  power: {
    whatItIs:
      'IDE extension with docs, MCP servers, and steering',
    whenToUse:
      'Rich IDE integrations with tool access and guided workflows',
    whenNotToUse: 'CLI-only workflows or simple text guidance',
    alternatives: ['skill', 'custom-agent'],
  },
  'mcp-server': {
    whatItIs:
      'Model Context Protocol server exposing tools via JSON-RPC',
    whenToUse:
      'Exposing external APIs or services to AI agents',
    whenNotToUse:
      'Simple file-based knowledge or static documentation',
    alternatives: ['power', 'custom-agent'],
  },
  'custom-agent': {
    whatItIs:
      'CLI agent with custom prompt, tools, and MCP servers',
    whenToUse:
      'CLI-specific workflows with custom tool configurations',
    whenNotToUse: 'IDE-only features or simple steering',
    alternatives: ['power', 'hook'],
  },
  'autonomous-agent': {
    whatItIs:
      'Cloud-hosted agent running tasks autonomously',
    whenToUse:
      'Long-running tasks, CI/CD integration, background work',
    whenNotToUse:
      'Interactive development or quick local operations',
    alternatives: ['custom-agent', 'hook'],
  },
  subagent: {
    whatItIs:
      'Delegated agent invoked by the main IDE agent',
    whenToUse:
      'Breaking complex tasks into focused sub-tasks',
    whenNotToUse:
  ...
  'context-provider': {
    whatItIs:
      '#-prefixed references injecting context into chat',
    whenToUse:
      'Providing files, folders, or specs as chat context',
    whenNotToUse:
      'Automated workflows or programmatic tool access',
    alternatives: ['steering-doc', 'skill'],
  },
};
